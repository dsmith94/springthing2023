...
function createCommonResponses() {
    ...
    // item manipulation
    cmd('y', [`yank`, `There's nothing here to yank.`])
    cmd('p', [`push`, `There's nothing here to push.`])
    cmd('b', [`yell`, `Your loud hollering accomplishes nothing.`])
    if (game.pepper) {
        cmd('g', [`grind pepper`, `Your surroundings are now more peppery.`])
    }
    if (game.whistle) {
        cmd('w', [`blow whistle`, `A shrill tone reverberates in the air.`])
    }
    cmd('a', [`attack`, `There's nothing here to attack.`])
    cmd('t', [`turn`, `There's nothing here worth rotating.`])
